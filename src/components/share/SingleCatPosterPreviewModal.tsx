    ...
    >
      ...
      >
        <div className="mb-3 flex items-center justify-between">
          <p className="text-base font-black text-[#221915]">分享這隻貓</p>
          <button
            type="button"
            onClick={onClose}
            aria-label="關閉分享預覽"
            className="grid h-9 w-9 place-items-center rounded-full border border-[#221915]/20 bg-white"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex min-h-[220px] items-center justify-center rounded-[18px] border border-[#221915]/12 bg-white/70">
          {previewUrl ? (
            <img src={previewUrl} alt="單貓分享海報預覽" className="max-h-[42vh] rounded-[14px] object-contain" />
          ) : (
        ...
        <section className="mt-3 rounded-[18px] border border-[#221915]/12 bg-white/82 p-3 shadow-[3px_3px_0_rgba(47,95,179,0.10)]">
          <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#2f5fb3]">分享頁預覽</p>
          <h3 className="mt-1 text-lg font-black leading-tight text-[#221915]">{previewTitle}</h3>
          {previewLocation ? (
        ...
        <label className="mt-3 block rounded-[14px] bg-white/70 p-3 text-sm font-bold text-[#221915]">
          <span className="block">出沒備註</span>
          <textarea
            aria-label="出沒備註"
            value={memoDraft}
            onChange={(event) => setMemoDraft(event.target.value)}
            placeholder={language === 'zh' ? '例如：下午常在轉角紙箱睡覺' : 'Example: often naps in the corner box'}
            className="mt-2 min-h-[72px] w-full resize-none rounded-[12px] border border-[#221915]/15 bg-[#fffdf2] px-3 py-2 text-sm font-semibold leading-relaxed text-[#221915] placeholder:text-[#8b7b6f] focus:outline-none focus:ring-2 focus:ring-[#3b82f6]/50"
          />
        </label>
